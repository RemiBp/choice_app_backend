...
const { restaurationDb, loisirCultureDb, beautyWellnessDb, choiceAppDb } = require('../index');
...
const searchController = {
  /**
   * Recherche globale sur toutes les collections
   */
  globalSearch: async (req, res) => {
    try {
      ...
      if (query) {
        ...
        // Recherche dans les utilisateurs (pas de géolocalisation)
        if (typesToSearch.includes('users')) {
          searchPromises.push(
            UserChoice.find(userTextQuery)
              .limit(parseInt(limit))
