...
const RestaurantProducer = restaurationDb.model('Producer', new mongoose.Schema({}), 'producers');
...
const producerController = {
  ...
  getProducerRelations: async (req, res) => {
    try {
      ...
      // Vérifier que le producteur existe
      const producer = await RestaurantProducer.findById(producerId).select(
      ...
      const data = {
        followers: {
          ...
          users: producer.followers?.map((id) => id.toString()) || [],
        },
        following: {
          count: producer.following?.length || 0,
          users: producer.following?.map((id) => id.toString()) || [],
        },
        choiceUsers: {
          count: producer.choiceUsers?.length || 0,
          users: producer.choiceUsers?.map(({ userId }) => userId.toString()) || [],
        },
        interestedUsers: {
          count: producer.interestedUsers?.length || 0,
          users: producer.interestedUsers?.map((id) => id.toString()) || [],
        },
      };
