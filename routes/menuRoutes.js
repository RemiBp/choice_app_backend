...
// Create schema for pending menu changes
const pendingMenuChangesSchema = new mongoose.Schema({
  ...
  modificationsHistory: { type: Map, of: [String] }
});
...
router.get('/producers/:producerId/menu/pending', auth, async (req, res) => {
  try {
    ...
    const pendingChanges = await PendingMenuChanges.findOne({ producerId });
    ...
    res.status(200).json({
      pending_approval: pendingChanges.pendingApproval,
      last_modified: pendingChanges.lastModified,
      modifications_history: pendingChanges.modificationsHistory || {}
    });
  } catch (error) {
...
router.post('/producers/:producerId/menu', auth, async (req, res) => {
  try {
    ...
    let pendingChanges = await PendingMenuChanges.findOne({ producerId });
    ...
    pendingChanges.menus = menus;
    pendingChanges.pendingApproval = pending_approval !== false; // Default to true
    pendingChanges.lastModified = last_modified ? new Date(last_modified) : new Date();
    
    // Update modifications history if provided
    if (modifications_history) {
      pendingChanges.modificationsHistory = modifications_history;
    }
...
router.post('/producers/:producerId/menu/approve', auth, async (req, res) => {
  try {
    ...
    // Update the menus
    producer.structured_data['Menus Globaux'] = pendingChanges.menus;
    
    // Save the updated producer
    await producer.save();
    
    // Update pending changes status
    pendingChanges.pendingApproval = false;
    await pendingChanges.save();
...
router.post('/producers/:producerId/menu/reject', auth, async (req, res) => {
  try {
    ...
    await PendingMenuChanges.deleteOne({ producerId });
